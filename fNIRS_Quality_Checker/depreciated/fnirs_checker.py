...
class fNIRSChecker(object):
    def __init__(self, data_dir="./data_to_check/joe_fnirs", signal_threshold=.07):
    ...
    def export_fnirs_csv(self, fnirs_data, time_series, fname, append_meta_data=True):
        ...
        if append_meta_data:
            ...
            fnirs_data["triggers"], start_trigger, end_trigger = self.create_trigger_col(fname, fnirs_data.shape[0])
    ...
    def create_fnirs_dataframe(self, file):
        ...
        return fnirs_data
        return fnirs_data, start_trigger, end_trigger
    ...
    def check_cv_of_channles(self):
        ...
        for i, file in enumerate(self.files):
            ...
            fnirs_data, start_trigger, end_trigger = self.export_fnirs_csv(fnirs_data, file["t"], self.fnames[i], append_meta_data=True)

            between_triggers = fnirs_data[start_trigger:end_trigger]
            between_triggers = between_triggers.drop(['time'], axis=1)
            between_triggers.plot()
            plt.show()
            ...
            fnirs_data_plot = fnirs_data.drop(['time'], axis=1)
            fnirs_data_plot.plot()
            plt.show()
